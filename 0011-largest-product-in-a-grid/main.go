...
<p class="monospace center narrow_device_shrink copy_to_clipboard">
...
func main() {
	...
	for m := range grid {
		for n := range grid {
			currentProduct := findMaxFourProduct(m, n, grid)
...
func findMaxFourProduct(m int, n int, grid [][]int) int {
	product := 1
	...
	if m+3 < len(grid) && n+3 < len(grid) {
		diaProduct := 1

		for i := range 4 {
			diaProduct *= grid[m+i][n+i]
		}

		if diaProduct > product {
			product = diaProduct
		}
	}

	if m-3 > 0 && n+3 < len(grid) {
		diaProduct := 1
